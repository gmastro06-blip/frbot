    ...
    *,
    ...
    try:
        ...
        if not before_ppm or not after_ppm:
            ...
            after_scan = None
            try:
                items = sorted(evidence_dir.glob('trade_basic_*_before.ppm'))
                if items:
...
def _load_config_from_env() -> RuntimeConfig:
    backend = (_env_str('FRBOT_TRADE_BASIC_BACKEND', 'real') or 'real').strip().lower()
...
def run_trade_basic_only() -> int:
    """trade_basic feature gate.
    ...
    try:
        ...
        outcome = execute_trade_tick(ctx, capture=capture, input_=input_, binding=binding, tick_index=0, gate='trade_basic')
        ev = outcome.evidence
        ...
        if dump_force or dump_enabled():
            before, after = snapshot('trade_basic')
            if before is not None or after is not None:
                before_ppm, after_ppm = dump_pair(gate='trade_basic', before=before, after=after, reason=str(ev.status))
        ...
        log_json(logger, event='success', gate='trade_basic', status='SUCCESS', intent_type=str(cfg.trade_action))
        ...
        if dump_force or dump_enabled():
            before, after = snapshot('trade_basic')
            if before is not None or after is not None:
                before_ppm, after_ppm = dump_pair(gate='trade_basic', before=before, after=after, reason=str(exc))
            else:
                try_dump_window_frame(gate='trade_basic', reason=str(exc))
...
if __name__ == '__main__':
    raise SystemExit(run_trade_basic_only())

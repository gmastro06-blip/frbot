...
from runtime.trade_full_preflight import run as trade_full_preflight_run
from runtime.trade_full_runner import execute_trade_full_tick
...
def _try_write_last_result(
    *,
    ...
    try:
        ...
        (evidence_dir / 'trade_last_result.json').write_text(
            json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + '\n',
            encoding='utf-8',
        )
    except Exception:
...
def _load_config_from_env() -> RuntimeConfig:
    ...
    return RuntimeConfig(
        ...
        trade_npc_roi=_env_str('FRBOT_TRADE_NPC_ROI', 'npc_trade_text'),
        trade_action_roi=_env_str('FRBOT_TRADE_ACTION_ROI', 'trade_action'),
        # Keep trade_inventory ROI for preflight reuse (trade_basic_preflight validates UI).
        trade_inventory_roi=_env_str('FRBOT_TRADE_INVENTORY_ROI', 'trade_inventory'),
...
def run_trade_full_only() -> int:
    ...
    Contract:
    - BEFORE capture -> exactly 1 input -> AFTER capture.
    - PASS only with semantic evidence: gold_after > gold_before.
    - In PROD-EMERGENCY: dumps *_before.ppm and *_after.ppm ALWAYS.
    """

    ctx: RuntimeContext | None = None
    action_kind = 'sell_all_creature_products'
    ...
    try:
        ...
        capture, input_, binding = trade_full_preflight_run(ctx)

        logger = configure_logger()
        ctx.status.state = RuntimeState.RUNNING

        outcome = execute_trade_full_tick(ctx, capture=capture, input_=input_, binding=binding, tick_index=0, gate='trade')
        ev = outcome.evidence
        ...
        if dump_force or dump_enabled():
            ...
            if before is not None or after is not None:
                before_ppm, after_ppm = dump_pair(gate='trade', before=before, after=after, reason=str(ev.status))
        ...
        _try_write_last_result(
            ...
            inventory_before=ev.inventory_before,
            inventory_after=ev.inventory_after,
            gold_before=ev.gold_before,
            gold_after=ev.gold_after,
        )

        log_json(logger, event='success', gate='trade_full', status='SUCCESS')
        return 0
        ...
        if dump_force or dump_enabled():
            before, after = snapshot('trade')
            if before is not None or after is not None:
                before_ppm, after_ppm = dump_pair(gate='trade', before=before, after=after, reason=str(exc))
            else:
        ...
        inv_b = getattr(getattr(ctx, 'trade', None), 'last_inventory_before', None) if ctx is not None else None
        inv_a = getattr(getattr(ctx, 'trade', None), 'last_inventory_after', None) if ctx is not None else None
        gold_b = int(inv_b.slot_counts.get('gold', 0)) if inv_b is not None else None
        gold_a = int(inv_a.slot_counts.get('gold', 0)) if inv_a is not None else None
